//Открытки и конверты
//	Для поздравлений  сотрудников фирмы было куплено N открыток и М конвертов.
//  Конверты и открытки оказались разных размеров, и некоторые открытки помещаются не во все конверты. 
//  Написать программу, находящую такое распределение открыток по конвертам, при котором поздравления получат наибольшее количество людей.
//  В один конверт разрешается вкладывать не более одной открытки.
//	В первой строке входного файла записаны числа М и N.
//  Далее записаны высота и ширина каждого из М конвертов, затем — высота и ширина каждой из N открыток. 
//  Размеры конвертов и открыток являются натуральными числами, не превосходящими 32767, и разделяются пробелами и/или символами перевода строки.
//	Выведите в выходной файл целое число К — максимальное количество открыток, которые можно разложить по конвертам. 
//  Далее выведите К пар целых чисел, означающих номер открытки и номер конверта, в который ее необходимо положить.
//
//
